...
public class CorporateFormPOC {
	...
	public void fillingForm() throws IOException {
		...
		data = ExcelUtils.getCellData(testData_file_path, "Sheet5");
		...
		for(int i=0; i<data.length; i++) {
			...
			if(driver.findElement(btn_scheduleDemo).isEnabled()) {
				...
				ExcelUtils.setCellData(testData_file_path, "Sheet5", i+1, 3, "TRUE");
				actualResults.add("TRUE");
				continue;
				
			} else {
				ExcelUtils.setCellData(testData_file_path, "Sheet5", i+1, 3, "FALSE");
	...
	public void compareResults() throws IOException {
		...
		System.out.println(ExcelUtils.getExpectedData(testData_file_path, "Sheet5").size());
		
		expectedResults = ExcelUtils.getExpectedData(testData_file_path, "Sheet5");
		
		for(int i=0; i<actualResults.size(); i++) {
			
			if(actualResults.get(i).equals(expectedResults.get(i))) {
				
				ExcelUtils.setCellData(testData_file_path, "Sheet5", i+1, 4, "Passed");
				ExcelUtils.fillGreenColor(testData_file_path, "Sheet5", i+1, 4);
				
			} else {
				
				ExcelUtils.setCellData(testData_file_path, "Sheet5", i+1, 4, "Failed");
				ExcelUtils.fillRedColor(testData_file_path, "Sheet5", i+1, 4);
			}
		}
	}
